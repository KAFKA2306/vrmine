...
public static class WoodlandTabletopVillageVerification
{
    ...
    public static void VerifyOrThrow()
    {
        ...
        Require(GameObject.Find("WoodlandAccents") != null, "woodland accent group is missing");
        Require(GameObject.Find("StreamReeds") != null, "stream reeds are missing");
        Require(GameObject.Find("MossyStump") != null, "mossy stump detail is missing");
        Require(GameObject.Find("WildflowerPatch") != null, "wildflower detail is missing");
        Require(GameObject.Find("VillageStoryTraces") != null, "village story detail group is missing");
        Require(GameObject.Find("OpenBook") != null, "open book life trace is missing");
        Require(GameObject.Find("Birdhouse") != null, "birdhouse detail is missing");
        Require(GameObject.Find("EnvironmentReadability") != null, "environment readability group is missing");
        Require(GameObject.Find("BridgeSignpost") != null, "bridge signpost detail is missing");
        Require(GameObject.Find("LeafLitter") != null, "leaf litter detail is missing");
        Require(GameObject.Find("ExhibitionPolish") != null, "exhibition polish group is missing");
        Require(GameObject.Find("PlazaStoneRing") != null, "plaza stone ring is missing");
        Require(GameObject.Find("FestivalBunting") != null, "festival bunting is missing");
    ...
    static void VerifyInteractionContract(Spec spec)
    {
        ...
        Require(spec.acceptance.must_not_include_yet != null && spec.acceptance.must_not_include_yet.Length > 0,
            "canonical must_not_include_yet contract is missing");
        ...
        foreach (string token in spec.acceptance.must_not_include_yet)
        {
            ...
            {
                Require(!Normalize(obj.name).Contains(normalizedToken),
                    "forbidden interaction/gameplay object is present: " + token + " -> " + obj.name);
                foreach (Component component in obj.GetComponents<Component>().Where(c => c != null))
                    Require(!Normalize(component.GetType().Name).Contains(normalizedToken),
                        "forbidden interaction/gameplay component is present: " + token + " -> " + component.GetType().Name);
            }
        }
    }
