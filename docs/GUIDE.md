...
2. `Assets/KafkaMade/VRMine/` でシーンと UdonSharp スクリプトを確認。Inspector で参照が Missing になっていないか必ずチェック。初期配置は `Assets/KafkaMade/VRMine/Editor/BootstrapScene.cs` か `Assets/KafkaMade/VRMine/Editor/VRMineBridge.cs` から整える。
3. VRChat Control Panel から「Build & Test」を押し、ローカルで同期挙動をテスト。ネットワークで気になる点は `docs/testing.md` にメモ。

## UdonSharp 復旧
- `VRChat SDK > Udon Sharp > Refresh All UdonSharp Assets` を実行し、プログラム資産の再生成を確認する。
- `Assets/KafkaMade/VRMine/Scenes/MVP.unity` を開き、`GameController` と `LogBoard` の参照が残っているか確認する。
