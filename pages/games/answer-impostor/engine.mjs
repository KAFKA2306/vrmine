...
export function validatePlayers(names) {
  ...
  if (normalized.length < 4 || normalized.length > 8) {
    return { ok: false, error: 'プレイヤーは4〜8人にしてください。' };
  }
  const folded = normalized.map((name) => name.toLocaleLowerCase('ja-JP'));
  if (new Set(folded).size !== normalized.length) {
    return { ok: false, error: '同じ名前は使用できません。' };
  }
...
export function createGame({ names, totalRounds = 5, seed = Date.now() }) {
  ...
  if (!Number.isInteger(rounds) || rounds < 3 || rounds > 8) {
    throw new Error('ラウンド数は3〜8にしてください。');
  }
  return {
    version: 1,
    seed: Number(seed) >>> 0,
...
export function startRound(game, questions = DEFAULT_QUESTIONS) {
  ...
  const candidates = selectQuestionCandidates(questions, 3, random);
  return {
    ...game,
...
export function chooseQuestion(questionCandidates, questionVotes, random = Math.random) {
  ...
  Object.values(questionVotes).forEach((question) => {
    if (Object.hasOwn(counts, question)) counts[question] += 1;
  });
...
export function scoreRound({ players, impostorId, targetId, answerOrder, votes }) {
  ...
  if (impostorIsLeader && leaders.length === 1) {
    impostorOutcome = 'failed';
    impostorPoints = 0;
  } else if (impostorIsLeader) {
    impostorOutcome = 'partial';
    impostorPoints = 1;
  }
  ...
  for (const player of players) {
    if (player.id === impostorId) continue;
    if (votes[player.id] === impostorId) {
      points[player.id] += 2;
...
export function applyRoundResult(game) {
  ...
  const players = game.players.map((player) => ({
    ...player,
    score: player.score + (result.points[player.id] ?? 0)
  }));
  ...
  return {
    ...
    history: [...game.history, completedRound],
    status: 'round-result'
  };
}
